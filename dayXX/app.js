  ...
  function mainCanvas( p ) {
    ...
    p.draw = function() {
      ...
      for (var i = 0; i < planets.length; i++) {
        ...
        planet.orbit = (planet.orbit + planet.orbitSpeed) % p.TWO_PI;
        var x = planet.orbitDist * p.cos(planet.orbit);
        var y = planet.orbitDist * p.sin(planet.orbit);
    ...
    function createSystem() {
      ...
      var sun = new Planet(p, {type: 'star'});
      sun.setSize(70 * (1 + p.random()*0.5 ));
      sun.setLocation(0, 0);

      planets.push(sun);

      var nbrOfPlanets = 1 + p.floor(p.random() * 10);
      var minDist = sun.size.x / 2 + 50;
      for (var i = 0; i < nbrOfPlanets; i++) {
        var newPlanet = new Planet(p, {type: 'crater'});
        newPlanet.setSize(10 + p.floor(p.random()*30));
        newPlanet.orbit = p.random() * p.TWO_PI;
        newPlanet.orbitSpeed = 0.01 + p.random() * 0.03;
        newPlanet.orbitDist = minDist + 50 + p.floor(p.random()*100);
        minDist = newPlanet.orbitDist + (newPlanet.size.x / 2);
  ...
  Planet = (function(superClass) {
    ...
    Planet.prototype.display = function () {
      this.startDisplay();
        ...
        p.noStroke();
        // Hallo
        p.fill(p.color(255, 255, 255, (255*0.01) ));
